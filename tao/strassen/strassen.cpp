...
void strassenR(vector<vector<double>> &A, vector<vector<double>> &B,
  ...
  if (tam <= LEAF_SIZE) {
    ...
    vector<vector<double>> a11(newTam, inner), a12(newTam, inner),
        a21(newTam, inner), a22(newTam, inner), b11(newTam, inner),
        b12(newTam, inner), b21(newTam, inner), b22(newTam, inner),
        c11(newTam, inner), c12(newTam, inner), c21(newTam, inner),
        c22(newTam, inner), p1(newTam, inner), p2(newTam, inner),
        p3(newTam, inner), p4(newTam, inner), p5(newTam, inner),
        p6(newTam, inner), p7(newTam, inner), aResult(newTam, inner),
...
void read(string filename, vector<vector<double>> &matrix) {
  char line[MAXCHAR];
  FILE *matrixfile = fopen(filename.c_str(), "r");
  ...
  double a;
  for (int i = 0; fgets(line, MAXCHAR, matrixfile) != NULL; i++) {
